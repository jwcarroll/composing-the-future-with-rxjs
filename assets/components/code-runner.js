...
function evalCode(code, console, Observable, _, starWarsService) {
...
export default class CodeRunner extends React.Component {
  ...
  componentDidMount() {
    ...
    evalCode(code, this.consoleLogger, Rx.Observable, _, starWarsService);
    this.scrollToBottom();    
  }
      ...
      <div style={containerStyle}>
        {
          streamNames.map(name =>
            <div style={itemStyle} className="nice-scrollbar" key={name}>
              ...
              {streams[name].map((log, i) => <div className="animated flipInX" key={log.id}>{log.val}</div>)}
